...
function bindData(data) {
    ...
    //让利率
    if(data.Lir){
	    var url = util.GetNewUrl(HOST + "Operator/GetItemList?DictionaryCode=OrgPromise");
	    util.showBusy("请求中");
	    $.ajax({
		    url: url,
		    data: {
		    ...
		    success: function (result) {
			    ...
			    if(data.Lir == 0.05){
				    $("#rll").val("5%");
				    $("#rll").attr("name", 0.05);
				    $("#dowcs_a").attr("href", result.Data[0].Value);
			    }else if(data.Lir == 0.08){
				    $("#rll").val("8%");
				    $("#rll").attr("name", 0.08);
				    $("#dowcs_a").attr("href", result.Data[1].Value);
			    }else if(data.Lir == 0.1){
				    $("#rll").val("10%");
				    $("#rll").attr("name", 0.1);
				    $("#dowcs_a").attr("href", result.Data[2].Value);
			    }else if(data.Lir == 0.15){
				    $("#rll").val("15%");
				    $("#rll").attr("name", 0.15);
				    $("#dowcs_a").attr("href", result.Data[3].Value);
			    }
			    $("#dowcs").show();
		    },
		    error: function () {
...
$(function () {
    ...
    $("#ranglilv").on("click", function () {
        ...
        layer.open({
            ...
            , content: '<div class="drlv">' +
                '<div name="0.05" SortId="1" class="rlv">5%</div >' +
                '<div name="0.08" SortId="2" class="rlv">8%</div>' +
                '<div name="0.1" SortId="3" class="rlv">10%</div>' +
                '<div name="0.15" SortId="4" class="rlv">15%</div></div>'
        });
        $(".rlv").on("click", function () {
            
            $("#rll").val($(this).html());  //展示让利率
            $("#rll").attr("name",$(this).attr("name"));  //添加nama
            
            
	        //$("#dowcs_a").attr("name", $(this).attr("name"));
	
	        $("#dowcs_a").attr("SortId", $(this).attr("SortId"));
